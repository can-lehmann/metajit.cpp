...
rev_binop_x86_inst(Mov16Mem, mov16_mem, mov_mem_usedef, false, { byte(0x66); rex_opt(); byte(0x89); modrm(); })
...
binop_x86_inst(MovZX8To64, movzx8to64, mov_usedef, false, { rex_w(); byte(0x0f); byte(0xb6); modrm(); })
binop_x86_inst(MovZX16To64, movzx16to64, mov_usedef, false, { rex_w(); byte(0x0f); byte(0xb7); modrm(); })
...
unop_x86_inst(Div8, div8, { use(rm); }, true, { reg = Reg::phys(6); rex(); byte(0xf6); modrm(); })
...
binop_x86_inst(Xor64, xor64, binop_usedef, true, { rex_w(); byte(0x33); modrm(); })
...
unop_x86_inst(Shl64, shl64, binop_usedef, true, { reg = Reg::phys(4); rex_w(); byte(0xd3); modrm(); })

unop_x86_inst(Shr8, shr8, binop_usedef, true, { reg = Reg::phys(5); rex(); byte(0xd2); modrm(); })
unop_x86_inst(Shr16, shr16, binop_usedef, true, { reg = Reg::phys(5); byte(0x66); rex_opt(); byte(0xd3); modrm(); })
unop_x86_inst(Shr32, shr32, binop_usedef, true, { reg = Reg::phys(5); rex_opt(); byte(0xd3); modrm(); })
unop_x86_inst(Shr64, shr64, binop_usedef, true, { reg = Reg::phys(5); rex_w(); byte(0xd3); modrm(); })

unop_x86_inst(Sar8, sar8, binop_usedef, true, { reg = Reg::phys(7); rex(); byte(0xd2); modrm(); })
unop_x86_inst(Sar16, sar16, binop_usedef, true, { reg = Reg::phys(7); byte(0x66); rex_opt(); byte(0xd3); modrm(); })
unop_x86_inst(Sar32, sar32, binop_usedef, true, { reg = Reg::phys(7); rex_opt(); byte(0xd3); modrm(); })
unop_x86_inst(Sar64, sar64, binop_usedef, true, { reg = Reg::phys(7); rex_w(); byte(0xd3); modrm(); })

imm_binop_x86_inst(Shl64Imm, shl64_imm, imm_usedef, true, { reg = Reg::phys(4); rex_w(); byte(0xc1); modrm(); imm_n(1); })

imm_binop_x86_inst(Shr8Imm, shr8_imm, imm_usedef, true, { reg = Reg::phys(5); rex(); byte(0xc0); modrm(); imm_n(1); })
imm_binop_x86_inst(Shr16Imm, shr16_imm, imm_usedef, true, { reg = Reg::phys(5); byte(0x66); rex_opt(); byte(0xc1); modrm(); imm_n(1); })
imm_binop_x86_inst(Shr32Imm, shr32_imm, imm_usedef, true, { reg = Reg::phys(5); rex_opt(); byte(0xc1); modrm(); imm_n(1); })
imm_binop_x86_inst(Shr64Imm, shr64_imm, imm_usedef, true, { reg = Reg::phys(5); rex_w(); byte(0xc1); modrm(); imm_n(1); })

imm_binop_x86_inst(Sar8Imm, sar8_imm, imm_usedef, true, { reg = Reg::phys(7); rex(); byte(0xc0); modrm(); imm_n(1); })
imm_binop_x86_inst(Sar16Imm, sar16_imm, imm_usedef, true, { reg = Reg::phys(7); byte(0x66); rex_opt(); byte(0xc1); modrm(); imm_n(1); })
imm_binop_x86_inst(Sar32Imm, sar32_imm, imm_usedef, true, { reg = Reg::phys(7); rex_opt(); byte(0xc1); modrm(); imm_n(1); })
imm_binop_x86_inst(Sar64Imm, sar64_imm, imm_usedef, true, { reg = Reg::phys(7); rex_w(); byte(0xc1); modrm(); imm_n(1); })

rev_binop_x86_inst(Cmp8, cmp8, cmp_usedef, false, { rex(); byte(0x38); modrm(); })
rev_binop_x86_inst(Cmp16, cmp16, cmp_usedef, false, { byte(0x66); rex_opt(); byte(0x39); modrm(); })
rev_binop_x86_inst(Cmp32, cmp32, cmp_usedef, false, { rex_opt(); byte(0x39); modrm(); })
rev_binop_x86_inst(Cmp64, cmp64, cmp_usedef, true, { rex_w(); byte(0x39); modrm(); })

imm_binop_x86_inst(Cmp8Imm, cmp8_imm, cmp_imm_usedef, false, { reg = Reg::phys(7); rex(); byte(0x80); modrm(); imm_n(1); })
imm_binop_x86_inst(Cmp16Imm, cmp16_imm, cmp_imm_usedef, false, { reg = Reg::phys(7); byte(0x66); rex_opt(); byte(0x81); modrm(); imm_n(2); })
...
unop_x86_inst(SetA8, seta8, { def(rm); }, false, { rex(); byte(0x0f); byte(0x97); modrm(); })
...
jmp_x86_inst(Jmp, jmp, {}, true, { byte(0xe9); imm_n(4); })
...
binop_x86_inst(AddSS, addss, binop_usedef, false, { byte(0xf3); rex_opt(); byte(0x0f); byte(0x58); modrm(); })
binop_x86_inst(SubSS, subss, binop_usedef, false, { byte(0xf3); rex_opt(); byte(0x0f); byte(0x5c); modrm(); })
binop_x86_inst(MulSS, mulss, binop_usedef, false, { byte(0xf3); rex_opt(); byte(0x0f); byte(0x59); modrm(); })
binop_x86_inst(DivSS, divss, binop_usedef, false, { byte(0xf3); rex_opt(); byte(0x0f); byte(0x5e); modrm(); })

binop_x86_inst(AddSD, addsd, binop_usedef, true, { byte(0xf2); rex_opt(); byte(0x0f); byte(0x58); modrm(); })
binop_x86_inst(SubSD, subsd, binop_usedef, true, { byte(0xf2); rex_opt(); byte(0x0f); byte(0x5c); modrm(); })
binop_x86_inst(MulSD, mulsd, binop_usedef, true, { byte(0xf2); rex_opt(); byte(0x0f); byte(0x59); modrm(); })
binop_x86_inst(DivSD, divsd, binop_usedef, true, { byte(0xf2); rex_opt(); byte(0x0f); byte(0x5e); modrm(); })

binop_x86_inst(MovSS, movss, mov_usedef, false, { byte(0xf3); rex_opt(); byte(0x0f); byte(0x10); modrm(); })
binop_x86_inst(MovSD, movsd, mov_usedef, true, { byte(0xf2); rex_opt(); byte(0x0f); byte(0x10); modrm(); })

rev_binop_x86_inst(MovSSMem, movss_mem, mov_mem_usedef, false, { byte(0xf3); rex_opt(); byte(0x0f); byte(0x11); modrm(); })
rev_binop_x86_inst(MovSDMem, movsd_mem, mov_mem_usedef, true, { byte(0xf2); rex_opt(); byte(0x0f); byte(0x11); modrm(); })
